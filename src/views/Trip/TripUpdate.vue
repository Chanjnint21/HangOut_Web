<template>
  <v-container class="MainContain">
    <v-card class="rounded-xl">
      ...
      <form-component ref="form" v-model="form" :timeData="time" :DateData="tripDate" @testing="passimagedata" :crrUser="crrUser">
...
export default {
  ...
  methods: {
    ...
    async UpdateData (id) {
      ...
      try {
        await C_Service.UpdateCard(id, form)
        this.$router.back()
      } catch (e) {
        console.log(e)
      }
  ...
  async created () {
    try {
      ...
      const ImportData = await C_Service.thisIdData(this.updateID)
